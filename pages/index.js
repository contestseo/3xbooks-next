...
export default function Home() {
  ...
  const fetchCategoryBooks = useCallback(
    (catId) => {
      return axios
        .get(`${API_BASE}/api/books?category=${catId}`)
        .then((res) => {
    ...
    const fetchAllData = async () => {
      try {
        const [catRes, authRes] = await Promise.all([
          ...
          axios.get(`${API_BASE}/api/authors`),
        ]);
        ...
        if (catRes.data.length > 0) {
          await fetchCategoryBooks(catRes.data[0]._id);
        }
  ...
  }, [API_BASE, fetchCategoryBooks]);

  const authorsWithBookCount = authors.map((a) => ({
    ...a,
    bookCount: a.books ? a.books.length : 0,
  }));
  ...
  // Optional: show preloader while loading
  // if (loading) return <Preloader fullPage={true} />;
...
export async function getServerSideProps() {
  ...
  try {
    ...
    if (categoriesSSR.length > 0) {
      const firstCategoryId = categoriesSSR[0]._id;
      const booksRes = await fetch(`${API_BASE}/api/books?category=${firstCategoryId}`);
